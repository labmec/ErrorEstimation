...
/**
 * @brief Returns a solution to multiphysics simulation.
...
void TPZMixedHDivErrorEstimate<MixedMat>::Errors(const TPZVec<TPZMaterialDataT<STATE> > &data, TPZVec<REAL> &errors)
{
    ...
    /**
     * data[0]= Reconstructed Hdiv 
     * data[1]= Reconstructed Primal 
     * data[2]= FEM Hdiv 
     * data[3]= FEM Primal
     * 
     * errors[0] = |Exact Primal - FEM Primal|^2
     * errors[1] = |FEM Primal - Reconstructed Primal|^2
     * errors[2] = |Exact Hdiv - FEM HDiv|^2
     * errors[3] = |FEM Hdiv - Reconstructed HDiv|^2
     * errors[4] = |Exact div(Hdiv) - FEM div(Hdiv)|^2
     **/
    
    const TPZVec<TPZMaterialDataT<STATE>> dataReconstructed(2);
    dataReconstructed[0] = data[0];
    dataReconstructed[1] = data[1];
    TPZVec<REAL> errorsReconstructed;
    /**
     * datavec[0]= Flux
     * datavec[1]= Pressure
     *
     ...
     * [4] Hdiv norm
     */
    this->Errors(dataReconstructed, errorsReconstructed);
    
    const TPZVec<TPZMaterialDataT<STATE>> dataFEM(2);
    dataFEM[0] = data[2];
    dataFEM[1] = data[3];
    TPZVec<REAL> errorsFEM;
    /**
     * datavec[0]= Flux
     * datavec[1]= Pressure
     *
     ...
     * [4] Hdiv norm
     */
    this->Errors(dataFEM, errorsFEM);
    
    errors.Resize(NEvalErrors());
    errors.Fill(0.0);

//    const int dim = MixedMat::Dimension();
//
...
template class TPZMixedHDivErrorEstimate<TPZMixedElasticityND>;
