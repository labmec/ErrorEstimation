...
void PrintNRefinementsByType(int nref, int64_t nels,int64_t newnels,TPZVec<int64_t> &counter,ostream &out = std::cout);
...
//	fileerrors << "\nType of element: " << typeel << endl;
	...
	if(gDebug) {
		sprintf(saida,"gmesh_%02dD_H%dE%dIndex.vtk",ModelDimension,ninitialrefs,typeel);
...
void PrintNRefinementsByType(int nref, int64_t nels,int64_t newnels,TPZVec<int64_t> &counter,ostream &out) {
