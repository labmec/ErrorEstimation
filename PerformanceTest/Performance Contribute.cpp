...
template<class TSHAPE>
void TimeComparaisonContribute();
template<class TSHAPE>
void TimeComparaisonContribute(int order);
...
int main(int argc, char *argv[]) {
    ...
    TimeComparaisonContribute<pzshape::TPZShapeTriang>();
    TimeComparaisonContribute<pzshape::TPZShapeLinear>();
    TimeComparaisonContribute<pzshape::TPZShapeQuad>();
    TimeComparaisonContribute<pzshape::TPZShapeTetra>();
    TimeComparaisonContribute<pzshape::TPZShapeCube>();
    TimeComparaisonContribute<pzshape::TPZShapePrism>();
...
void TimeComparaisonContribute()
{
    ...
    TimeComparaisonContribute<TSHAPE>(order);
    order = 2;
    std::cout << "order = " << order << std::endl;
    TimeComparaisonContribute<TSHAPE>(order);
    order = 3;
...
//    std::cout << "after " << cel->AllSideOrient() << std::endl;
    ...
    boost::posix_time::ptime::time_system_type::time_duration_type timenew1, timenew2, timeold;
...
//        cel->ComputeRequiredData(data_old[0],qsi);
    ...
    mixed->m_fast = 2;
    ek_new.Zero();
    boost::posix_time::ptime tsim4 = boost::posix_time::microsec_clock::local_time();
    for(int i=0; i<10000; i++)
    {
//        cel->InitMaterialDataNew(data_new[0]);
//        cel->ComputeRequiredDataNew(data_new[0],qsi);
    ...
    boost::posix_time::ptime tsim5 = boost::posix_time::microsec_clock::local_time();
...
//    std::cout << "Error ek " << err_ek << std::endl;
    ...
    timenew1 = tsim2-tsim1;
    timenew2 = tsim5-tsim4;
    timeold = tsim3-tsim2;
//    std::cout << "Sum wall time = " << tsim01 - tsim0 << " s" << std::endl;
    std::cout << "Total wall time of ComputeContribute old = " << timeold << " s" << std::endl;
    std::cout << "Total wall time of ComputeContribute new1 = " << timenew1 << " s" << std::endl;
    std::cout << "Total wall time of ComputeContribute new2 = " << timenew2 << " s" << std::endl;
