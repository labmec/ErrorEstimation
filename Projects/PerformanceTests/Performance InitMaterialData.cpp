...
template<class TSHAPE>
void TimeComparaisonInitMaterialData();
template<class TSHAPE>
void TimeComparaisonInitMaterialData(int order);
...
int main(int argc, char *argv[]) {
    ...
    TimeComparaisonInitMaterialData<pzshape::TPZShapeLinear>();
    TimeComparaisonInitMaterialData<pzshape::TPZShapeQuad>();
    TimeComparaisonInitMaterialData<pzshape::TPZShapeTriang>();
    TimeComparaisonInitMaterialData<pzshape::TPZShapePrism>();
    TimeComparaisonInitMaterialData<pzshape::TPZShapeTetra>();
    TimeComparaisonInitMaterialData<pzshape::TPZShapeCube>();
...
void TimeComparaisonInitMaterialData()
{
    ...
    TimeComparaisonInitMaterialData<TSHAPE>(order);
    order = 2;
    std::cout << "order = " << order << std::endl;
    TimeComparaisonInitMaterialData<TSHAPE>(order);
    order = 3;
...
//    std::cout << "before " << cel->AllSideOrient() << std::endl;
    ...
    {
        cel->AllSideOrient()[0]=-1;
    }
//    std::cout << "after " << cel->AllSideOrient() << std::endl;
    for(int i=0; i<=nc; i++) if(first_old[i] != first_new[i]) DebugStop();
    for(int i=0; i<nc; i++)
    {
        int nc_old = cel->NConnectShapeF(i, order);
        int nc_new = cel->NConnectShapeFNew(i, order);
        if(nc_old != nc_new) DebugStop();
    }
    TPZVec<TPZMaterialData> data_old(2),data_new(2);
    data_old[1].fShapeType = TPZMaterialData::EVecandShape;
    data_new[1].fShapeType = TPZMaterialData::EVecandShape;
...
//        mixed->Contribute(data_old, weight, ek_old, ef);
    ...
    for (int ivec = 0; ivec<nvec ; ivec++) {
        ...
        TPZManVector<REAL,3> vec_old(TSHAPE::Dimension),vec_new(TSHAPE::Dimension);
        for(int i=0; i<TSHAPE::Dimension; i++)
        {
            vec_old[i] = data_old[0].fMasterDirections(i,ind_old);
            vec_new[i] = data_new[0].fMasterDirections(i,ind_new);
        }
//        std::cout << "i " << ivec << " vec_old " << vec_old << " ind_old " << ind_old << std::endl;
//        std::cout << data_old[0].fMasterDirections(0,ind_old) << " " << data_old[0].fMasterDirections(1,ind_old) << " " << data_old[0].fMasterDirections(2,ind_old) << std::endl;
//        std::cout << "i " << ivec << " vec_new " << vec_new << std::endl;
        for(int i=0; i<TSHAPE::Dimension; i++)
        {
...
//    std::cout << "Sum wall time = " << tsim01 - tsim0 << " s" << std::endl;
    std::cout << "Total wall time of InitMaterial old = " << timeold << " s" << std::endl;
    std::cout << "Total wall time of InitMaterialData new = " << timenew << " s" << std::endl;
